...
```
  ...
  -p [P]        	If True, generates interactive PyMOL sessions and motif images for output subclusteres. Default: False.
...
5. __subcluster_images folder:__ contanis the interactive PyMOL sessions and motifs images generated for each subcluster. The images will be generated if PyMOL is installed and "-p" parameter is used while running Auto-GINClus.
...
### Important Notes   
*** The subcluster images and superimposed images of new RNA structurals motifs for [internal loops](output/Internal_Loop_Output/New_RNA_Motif_Subcluster_Images/) and [hairpin loops](output/Hairpin_Loop_Output/New_RNA_Motif_Subcluster_Images/) are provided inside folder [output](output/). Due to space limitation, we didn't provide the images and PyMOL sessions for all the subclusters (3525 internal loop and 2384 hairpin loop subclusters).  
