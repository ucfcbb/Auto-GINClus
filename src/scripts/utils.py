...
def parse_TM_alignment_block(lines, line_index):
    r1_line = lines[line_index]
    ...
    r1 = '_'.join(r1.strip().split('_')[:2]) + ':' + '_'.join(r1.strip().split('_')[2:])
    r2 = '_'.join(r2.strip().split('_')[:2]) + ':' + '_'.join(r2.strip().split('_')[2:])

    line = lines[line_index + 5]
    align_len_text, rmsd_text, seq_identity_text = line.strip().split(',')
    align_len = align_len_text.strip().split('=')[1].strip()
    rmsd = rmsd_text.strip().split('=')[1].strip()
    seq_identity = seq_identity_text.strip().split('=')[2].strip()

    #temporary
    tm_score_list = []
    TM_score_text = lines[line_index + 6]
    tm_score_list.append(TM_score_text.strip().split('(')[0].strip().split('=')[1].strip())

    TM_score_text = lines[line_index + 7]
    tm_score_list.append(TM_score_text.strip().split('(')[0].strip().split('=')[1].strip())

    TM_score_text = lines[line_index + 8]
    tm_score_list.append(TM_score_text.strip().split('(')[0].strip().split('=')[1].strip())
    #temporary

    TM_score_text = lines[line_index + 8]
    TM_score = TM_score_text.strip().split('(')[0].strip().split('=')[1].strip()

    aln1 = lines[line_index + 12].strip()
    aln2 = lines[line_index + 14].strip()

    #temporary
    return r1, r2, aln1, aln2, TM_score, align_len, rmsd, seq_identity, tm_score_list

    #original
    # return r1, r2, aln1, aln2, TM_score, align_len, rmsd, seq_identity
...
# def get_backbone_and_sugar_atom_beads_rbt(res):
    
#     phosphate_bead = ["P", "OP1", "OP2"]
#     sugar_bead = ["'C1''", "'C2''", "'C3''", "'C4''", "'O4''"]
#     phosphate_sugar_connect_bead = ["'O5''", "'C5''", "'C4''"]
#     base_bead = []
#     sugar_nitrogenbase_connect_bead = []

#     if res == "A" or res == "G":
#         base_bead = ["C2", "C4", "C6", "C8", "N9"]
#         sugar_nitrogenbase_connect_bead = ["'C1''", "N9"]
#     else:
#         base_bead = ["C2", "C4", "C6", "N1"]
#         sugar_nitrogenbase_connect_bead = ["'C1''", "N1"]

#     return phosphate_bead, sugar_bead, phosphate_sugar_connect_bead, base_bead, sugar_nitrogenbase_connect_bead


# def get_backbone_and_sugar_atom_beads(res):
...
# def get_backbone_and_sugar_atoms_rbt():
...
def get_backbone_and_sugar_atoms():
    ...
    # base atoms for Pyrimidines (Cytosine and Uracil) [N1, C2, C4, C6]
    # base atoms for Purines (Adenine and Guanine) [N9, C2, C6, C4, C8]
    # phosphate_bead = ["P", "OP1", "OP2"]
    # sugar_bead = ["C1'", "C2'", "C3'", "C4'", "O4'"]
    # base_bead = ["C2", "C4", "C6", "C8", "N1", "N9"]
    # base_bead = ["C2", "C4", "C6", "C8", "N1", "N9"]
    # purine_base_bead = ["C2", "C4", "C6", "C8", "N9"]
    # pyrimidines_base_bead = ["C2", "C4", "C6", "N1"]
    # phosphate_sugar_connect_bead = ["O5'", "C5'", "C4'"]
    # purine_sugar_nitrogenbase_connect_bead = ["C1'", "N9"]
    # pyrimidines_sugar_nitrogenbase_connect_bead = ["C1'", "N1"]
...
def convert_a_cluster_from_PDB_to_FASTA(families):
    ...
    # Nabila debug
    fout = open("Filter_loops.txt", "w")
    fout.close()
    No_filtered_loops_mapping_issues = 0
    ...
    for family_id in families:
        ...
        for loop in loops:
            ...
            # ### Skip loops with negative indexing
            # count1 = loop.count("_") 
            # count2 = loop.count("-")
            # if count2 != count1:
            #     logger.warning("Loop " + loop + " has negative indexing, filtered out.")
            #     continue
            ...
            # Nabila debug
            if "None" in loop_pdb:
                logger.warning(loop + " contains 'None' after PDB to Fasta conversion: " + loop_pdb + ". Skipping loop.") 
                fout = open("Filter_loops.txt", "a")
                fout.write(loop + ",")
                fout.close()
                No_filtered_loops_mapping_issues += 1
                continue

            families_pdb[family_id].append(loop_pdb)

    print("")
    logger.warning("Number of filtered loops due to PDB to Fasta mapping issues: " + str(No_filtered_loops_mapping_issues))
    return families_pdb
...
def convert_a_loop_from_PDB_to_FASTA(loop):
    ...
    for segment in segments:
        ...
        if segment.count('-') == 3:
            ...
            a, b = segment.strip().strip('-').split('-')
            a = '-' + a
        else: # expecting segment.count('-') == 1, otherwise, error will occur; need to investigate by case.
            a, b = segment.strip().split('-')
...
def cleanup_output_directories(removable_text_file_list, superimposition_output_dir, subfamilies_dir, superimposition_details_dir, pymol_session_dir, draw_figures):
...
def create_required_directories(partial_pdbx_dir, output_dir):
...
def filter_loops_containing_HETATM(families, partial_pdbx_dir):
    filtered_families = {}
    filtered_count = 0
    for family_id in families:
        loops = families[family_id]
        filtered_loops = []
        for loop in loops:
            loop_fn = os.path.join(partial_pdbx_dir, loop.replace(':', '_') + '.cif')
            fp = open(loop_fn)
            lines = fp.readlines()
            fp.close()
            has_HETATM = False
            ...
            if has_HETATM == False:
                ...
                filtered_count += 1
        if len(filtered_loops) > 0:
            filtered_families[family_id] = filtered_loops

    return filtered_families, filtered_count
...
def get_separated_sequences(loop, sequence):
    pdb_chain, segments = loop.strip().split(':')
    segments = segments.strip().split('_')
    seq_parts = []
    start_ind = 0
    for segment in segments:
        s, e = segment.strip().split('-')
        s = int(s)
        e = int(e)
        l = e - s + 1
        seq_parts.append(sequence[start_ind:start_ind+l])
