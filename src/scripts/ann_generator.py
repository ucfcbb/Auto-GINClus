...
def get_dssr_and_fr3d_merged_annotation_for_single_pdb(selected_annotation_dir, pdb_id, dssr_type):
	...
	if not os.path.isfile(dssr_fname):
		...
		if dssr_type == 'tool':
			...
			download_dssr_annotation_for_single_pdb(dssr_ann_dir, pdb_id)
	...
	if len(ann_list) == 0:
		logger.error('None of the DSSR and FR3D annotation files contain annotations. Cannot generate the merged annotation file for ' + pdb_id + '. Skipping.')
		# Nabila debug
		# sys.exit()
