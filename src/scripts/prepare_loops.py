...
def download_single_pdbx_or_fasta_file(fname, file_ext):
    ...
    while status == False and number_of_tries <= download_attempts:
        ...
        if file_ext == 'cif':
            ...
            response = requests.get(pdbx_url + fname, allow_redirects=False)
            
            if response.status_code == 200:
                fp = open(os.path.join(pdbx_dir, fname), 'wb')
                fp.write(response.content)
                fp.close()
            else:
                logger.error('Error downloading PDBX file')
                sys.exit()
            ...
            response = requests.get(fasta_url + fname.strip().split('.')[0], allow_redirects=False)

            if response.status_code == 200:
                fp = open(os.path.join(fasta_dir, fname), 'wb')
                fp.write(response.content)
                fp.close()
            else:
                logger.error('Error downloading FASTA file')
                sys.exit()
...
def _dssr_annotation_worker(p):
    ...
    # download_dssr_annotation_for_single_pdb(*p)
...
def get_annotation_files(pdb_list, dssr_type):
    ...
    if annotation_source.lower() == 'dssr':
        ...
        for pdb_id in pdb_list:
            ...
            if os.path.isfile(annotation_fname):
                ...
                parameter_list.append((selected_annotation_dir, pdb_id, dssr_type))
...
def generate_loop_files_for_single_pdb(pdb_id, chains, loops):
    ...
    for item in loops:
        ...
        for i in range(len(loop)):
            ...
            loop_fn_smf = os.path.join(loop_dir, pdb_id+'_%s_%s.smf' % (chain_id, '_'.join(map(lambda x: str(x[0])+'-'+str(x[1]), loop_i))))  #for scanx with stacking
            ...
            if chain_id in missing_residue_dict:# or chain_id in residue_with_missing_base_dict:
                ...
                if len(missing_res_segment_lengths) > 0:
                    ...
                    if missing_res_length > 5 or 2 * missing_res_length > len(loop_i_index):
                        ...
                        loop_fn_smf = os.path.join(loop_dir, 'missing_res/' + type, pdb_id+'_%s_%s.smf' % (chain_id, '_'.join(map(lambda x: str(x[0])+'-'+str(x[1]), loop_i))))  #for scanx with stacking
            if output_env == 'local':
                fms.close()
            ...
            # Nabila debug
            loop_stack_final_intra = []
            fs.write('#info=stacking\n')
            for i, j, intera in loop_stack_intera:
                if i < j:
                    # Nabila debug
                    if (i, j, intera[2]) not in loop_stack_final_intra:
                        loop_stack_final_intra.append((i, j, intera[2]))
                        fs.write('%d-%d,%s\n' % (i, j, intera[2]))
                else:
                    if intera[2] == 'upward':
                        # Nabila debug
                        if (j, i, 'downward') not in loop_stack_final_intra:
                            loop_stack_final_intra.append((j, i, 'downward'))
                            fs.write('%d-%d,%s\n' % (j, i, 'downward'))
                    elif intera[2] == 'downward':
                        # Nabila debug
                        if (j, i, 'upward') not in loop_stack_final_intra:
                            loop_stack_final_intra.append((j, i, 'upward'))                        
                            fs.write('%d-%d,%s\n' % (j, i, 'upward'))
                    else:
                        fs.write('%d-%d,%s\n' % (j, i, intera[2]))
...
def prepare_data(families, dssr_type):
    pdb_chains = {}
    # pdb_loops = {}
    ...
    get_annotation_files(pdb_list, dssr_type) #chkd
...
def prepare_loop_files(loop_node_list_str):
    ...
    for node in loop_node_list_str:
        ...
        for loop in loop_rotations:
            loop_fn_smf = os.path.join(loop_dir, loop.replace(':', '_') + '.smf')
