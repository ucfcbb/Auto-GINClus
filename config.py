...
save_pymol_session = True 				# Make it True to save PyMOL sessions of the superimposed motif instances.
...
#input_index_type = 'fasta'                # pdb / fasta [To define the indices type in input file. For FASTA, it is 0 indexed.]
...
output_env = 'local'   # global / local [global: cleaned up outputs; local: keep intermediate files]
# dssr_type = 'tool'        # web / tool [web: downloads from http://skmatic.x3dna.org/pdb/ website; tool: generates using dssr tool]          

# for analysis
is_length_adjusted_score = False
generate_bp_ann_files = False            # generate base-pair annotation file
...
generate_loop_source_info = False
show_cluster_source = False
...
# global problematic_tmalign_loops
problematic_tmalign_loops = []
# global problematic_scanx_alignments
problematic_scanx_alignments = []
##### DEVELOPER PARAMS ##### DEVELOPER PARAMS ##### DEVELOPER PARAMS ##### DEVELOPER PARAMS ##### DEVELOPER PARAMS #####
...
graph_and_pickles_dir = os.path.join(data_dir, 'graphs_and_pickles')
...
# graph_dir = os.path.join(root_dir, 'graphs_dir')

# libraries
dssr_dir = os.path.join(lib_dir, 'DSSR')
motifscanx_dir = os.path.join(lib_dir, 'RNAMotifScanX-release')
tmalign_dir = os.path.join(lib_dir, 'TMalign', 'TMalign-20180426')
...
candidate_data_path = 'graphs_dir/Candidate_Graph_Data/'
train_dt_path = 'GVPGINE_motif_hetero_dt'
candidate_dt_path = 'GVPGINE_loop_hetero_dt'
subcluster_images = 'output/subcluster_images'
subcluster_images_scanx = 'output/subcluster_images/scanx/'
subcluster_images_tmalign = 'output/subcluster_images/tmalign/'
